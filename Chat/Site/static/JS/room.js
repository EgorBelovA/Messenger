...
//                        if(response.all_chats[item].users[1] == response.my_user)
//                            opponent = response.all_chats[item].users[0];
//                        else
//                            opponent = response.all_chats[item].users[1];

                        var opponent_ind = 0;
                        var opponent_name = "";

                        if(new_room.room_type == "G")
                            opponent_name = new_room.name;
                        if(new_room.room_type == "D")
                            opponent_name = "IDK";
                        if(new_room.room_type == "C")
                            opponent_name = new_room.name;

//                        for(all_users_list in response.all_users){
//                            if(response.all_users[all_users_list].id == opponent){
//                                opponent_ind = all_users_list;
//                                break;
//                            }
//                        }
//                        opponent_name = response.all_users[opponent_ind].username;
...
//                        }
                        ...
                        if(new_room.image != '' && new_room.image != null){
                            var avatar = document.createElement("img");
                            avatar.src = "//" + window.location.host + "/media/" + new_room.image.slice(6);
                        ...
                        if(new_room.image != '' && new_room.image != null)
                            avatar.src = "//" + window.location.host + new_room.image.slice(6);
                        else{
                            var name = new_room.name;
                        ...
                        user_contact_id.value = new_room.id;
                        ...
                        user_contact_name.innerHTML = opponent_name;
//                        if(response.all_chats[item].room_type == 'G') user_contact_name.textContent = response.all_chats[item].name;
                        ...
                        connect_socket(new_room.id);

                        users_list.mousedown = function(){
...
//                                controller.abort();
                                ...
                                avatar_attachments.setAttribute("class", "avatar_attachments");
                                ...
                                avatar_attachments_username.textContent = document.querySelector("#name").value;

                                avatar_attachments.appendChild(avatar_attachments_username)

                                document.querySelector("#opponent_photo_avatar").firstChild.replaceWith(avatar_attachments);
                            }
                            else
